...
curl -X PATCH http://localhost:3000/api/v1/users/me \
  -H "Authorization: Bearer <accessToken>" -H "Content-Type: application/json" \
  -d '{"fullName": "Updated Name", "phone": "+2348012345678"}'

# Change password -- save the new accessToken/refreshToken from the response
curl -X POST http://localhost:3000/api/v1/auth/change-password \
  -H "Authorization: Bearer <accessToken>" -H "Content-Type: application/json" \
  -d '{"currentPassword": "your-current-password", "newPassword": "a-new-strong-password"}'

# Confirm the OLD password no longer works
curl -X POST http://localhost:3000/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"you@example.com","password":"your-current-password"}'
# Expect 401 AUTH_INVALID_CREDENTIALS
```
...
- `POST /auth/change-password` — requires the current password (a valid
  access token alone isn't enough — someone at an unlocked device
  shouldn't be able to lock the real owner out). On success, every OTHER
  refresh token for that user is revoked — a password change is exactly
  the moment you'd want a lost device or shared computer logged out — and
  a fresh token pair is issued so the session making the change keeps
  working. Covered by an e2e test that specifically proves a second
  session dies while the changing session survives, not just that the
  password itself updates.
- `GET /users/me` — any authenticated user
- `PATCH /users/me` — update your own name/phone. Deliberately does NOT
  allow changing email — that's a login identifier, not just contact
  info, and deserves its own verification flow if it's ever built, not a
  quiet field on a general profile form.
